...
const TABLE_HEAD = [
    { id: "title", label: "Title", align: "left", sort: true, width: "30%" },
...
const ListPage = () => {
    ...
    const { count, blogs, isLoading } = useSelector((state) => state?.blog);
    ...
    const handleEditRow = (id) => id ? router?.push(ROUTES?.ADMIN?.BLOGS?.EDIT(id)) : null;

    const handleDeleteRow = (recordId) => {
        if (recordId) {
            const onConfirm = () => {
                dispatch(deleteBlog(recordId))
    ...
    return (
        <PageWrapper title="Blog Management">
            <FlexBox mb={2} gap={2} justifyContent="space-between" flexWrap="wrap">
                <SearchInput placeholder="Search Blog..." onChange={e => handleSearch(e.target.value)} />
                <DarkButton href={ROUTES?.ADMIN?.BLOGS?.ADD} color="info" variant="contained" startIcon={<Add />} LinkComponent={Link}>Add Blog</DarkButton>
            </FlexBox>
            <Box sx={{ py: 1 }}>
