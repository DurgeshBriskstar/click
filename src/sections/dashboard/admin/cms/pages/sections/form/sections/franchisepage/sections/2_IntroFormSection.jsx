...
export default function IntroFormSection({ control, sectionKey, errors }) {
    const { fields: highlightFields, append, remove } = useFieldArray({ control, name: `sections.${sectionKey}.highlights` });

    const canAddMore = highlightFields.length < 3;
        ...
        <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
                <Grid size={{ xs: 12 }}>
                    <CTextField
                        control={control}
                        name={`sections.${sectionKey}.form_iframe`}
                        label="Highlevel Iframe"
                        placeholder="Paste iframe code here..."
                        multiline
                        rows={11.8}
                        error={errors?.sections?.[sectionKey]?.form_iframe}
                    />
                </Grid>
            </Grid>
            ...
            <Grid size={{ xs: 12 }}>
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                    <Typography variant="h6" fontWeight={600}>
                        Highlights ({highlightFields.length}/3)
                    </Typography>
                    <Button
                        type="button"
                        variant="outlined"
                ...
                <Grid container spacing={3} width='100%'>
                    {highlightFields.map((field, catIndex) => {
                        return (
                            ...
                            >
                                <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                                    <Typography variant="subtitle1" fontWeight={600}>Highlight {catIndex + 1}</Typography>
            ...
            <Grid size={{ xs: 12 }}>
                <Typography fontWeight={400}><b>Note: </b>Rranchise form displayed in this section will be sourced from the entered iframe.</Typography>
            </Grid>
        </Grid>
    );
};