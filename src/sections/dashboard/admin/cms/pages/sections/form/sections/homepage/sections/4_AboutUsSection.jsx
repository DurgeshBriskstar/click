...
export default function AboutUsSection({ control, sectionKey, errors }) {
        ...
        <Grid container spacing={3}>
            ...
            <Grid container spacing={3} width='100%'>
                {highlightFields.map((field, hiIndex) => {
                    const isFirst = hiIndex === 0;
                    return (
                        <Grid key={field.id} size={{ xs: 12, sm: 6, md: 3 }}
                            sx={{
                                p: 2.5,
                                border: '1px solid',
                                borderColor: 'divider',
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
                                ...
                                {isFirst && (
                                    <>
                                        <CTextField
                                            control={control}
                                            name={`sections.${sectionKey}.highlights.${hiIndex}.description`}
                                            label="Subtitle"
                                            placeholder="Enter highlight subtitle"
                                            multiline
                                            rows={3}
                                            error={errors?.sections?.[sectionKey].highlights?.[hiIndex]?.description}
                                        />
                                        <CTextField
                                            control={control}
                                            name={`sections.${sectionKey}.highlights.${hiIndex}.buttonText`}
                                            label="Button Text"
                                            placeholder="Enter button text"
                                            error={errors?.sections?.[sectionKey].highlights?.[hiIndex]?.buttonText}
                                        />
                                        <CTextField
                                            control={control}
                                            name={`sections.${sectionKey}.highlights.${hiIndex}.buttonLink`}
                                            label="Button Link"
                                            placeholder="Enter button link"
                                            error={errors?.sections?.[sectionKey].highlights?.[hiIndex]?.buttonLink}
                                        />
                                    </>
                                )}
                            </Box>
