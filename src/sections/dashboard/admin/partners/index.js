...
import { deletePartner, getPartners } from "store/slices/partnerSlice";
...
const TABLE_HEAD = [
    { id: "partner_name", label: "Name", align: "left", sort: true },
...
const ListPage = ({ }) => {
    ...
    const { count, partners, isLoading } = useSelector((state) => state?.partner)
    ...
    const handleEditRow = (id) => id ? router?.push(ROUTES?.ADMIN?.PARTNER?.EDIT(id)) : null;

    const handleDeleteRow = (recordId) => {
        if (recordId) {
            const onConfirm = () => {
                dispatch(deletePartner(recordId))
    ...
    return (
        <PageWrapper title="Partner Management">
            <FlexBox mb={2} gap={2} justifyContent="space-between" flexWrap="wrap">
                <SearchInput placeholder="Search Partner..." onChange={e => handleSearch(e.target.value)} />
                <DarkButton href={ROUTES?.ADMIN?.PARTNER?.ADD} color="info" variant="contained" startIcon={<Add />} LinkComponent={Link}>Add Partner</DarkButton>
            </FlexBox>
            <Box sx={{ py: 1 }}>
