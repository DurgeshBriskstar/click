...
export default function FormPage({ isEdit = false }) {
    ...
    const handleBack = () => router?.push(ROUTES?.ADMIN?.SERVICE?.LIST);

    useEffect(() => {
        if (isEdit && id) {
            dispatch(getService(id)).then((resp) => {
                if (!resp?.data) {
                    handleBack();
    ...
    return (
        <PageWrapper title={isEdit ? "Edit Service" : "Create Service"}>
            <Box sx={{ py: 1 }}>
                {isLoading && <LinearProgress sx={{ color: "#4e96fe" }} color="inherit" />}
            </Box>
            <Form isEdit={isEdit} currentRecord={id ? service : {}} onBack={handleBack} />
        </PageWrapper>
    );
}