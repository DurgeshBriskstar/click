...
export default function DetailSection2({ data }) {
  ...
  return (
    <section className="section detail-section-band">
      <Container>
        <Row className="align-items-center">
          <Col xs={12} md={12} lg={6} className="order-1 order-md-2" data-aos="fade-left" suppressHydrationWarning>
            {(data?.image || data?.background_image) && (
              <div className="store-contact-card">
                {(data?.image || data?.background_image) && (
                  <div className="store-contact-card-image-wrapper">
                    <div className="store-contact-card-image">
                      <img src={getFileFullPath(data.image || data.background_image)} alt={data?.title || "Service Detail"} />
                    </div>
                  </div>
                )}
          ...
          <Col xs={12} md={12} lg={6} className="order-2 order-md-1" data-aos="fade-right" suppressHydrationWarning>
            ...
            {data?.highlights && data.highlights.length > 0 && (
              <ul className="points-list black-text">
                {data.highlights.map((highlight, index) => (
                  <li key={index} data-aos="fade-up" data-aos-delay={index * 100} suppressHydrationWarning>
